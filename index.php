...
if ($method=='GET') {
    ...
    echo isset($_REQUEST['hub_challenge'])?$_REQUEST['hub_challenge']:"SSS";
} elseif ($method == 'POST') {
	$input = file_get_contents('php://input');

	$data = json_decode($input, true);
	$r= print_r($data,true);
	$filepath="/Applications/MAMP/htdocs/messenger/temp.txt";
    ...
    if (!empty($data['entry'][0]['messaging'])) {
        ...
        foreach ($data['entry'][0]['messaging'] as $message) {
            ...
            if (isset($message['message']['is_echo']) and ($message['message']['is_echo'] == "true")) {
                continue;
            }
            ...
            switch ($command) {
                ...
                case 'how much charge for accounting ?':
                    $bot->send(new Message($message['sender']['id'], 'We are charge for $500 SGD per months'));
                    break;

	            // When bot receive "text"
	            case 'hi':
		            $bot->send(new Message($message['sender']['id'], 'welcome Jhonsan Vettula yellarum nalama'));
		            break;
	            case 'மதிக்கவே இல்லைய':

		            $bot->send(new Message($message['sender']['id'], 'வெளிய போங்கடா அய்யோக்கிய ராஸ்கல் '));
		            break;
	            ...
	            // When bot receive "image"
	            case 'audio':
		            $bot->send(new AudioMessage($message['sender']['id'], 'http://audio-ssl.itunes.apple.com/apple-assets-us-std-000001/Music49/v4/2f/60/92/2f609226-e3dd-e22d-211e-dcc248b7a2dd/mzaf_6823002190713253331.plus.aac.p.m4a'));
		            break;
	            // When bot receive "image"
	            case 'illaiyaraja':
		            $bot->send(new AudioMessage($message['sender']['id'], 'http://audio-ssl.itunes.apple.com/apple-assets-us-std-000001/Music49/v4/2f/60/92/2f609226-e3dd-e22d-211e-dcc248b7a2dd/mzaf_6823002190713253331.plus.aac.p.m4a'));
	            ...
	            // When bot receive "image"
	            case 'raja':
		            $bot->send(new AudioMessage($message['sender']['id'], 'https://audio-ssl.itunes.apple.com/apple-assets-us-std-000001/AudioPreview71/v4/b9/3a/4a/b93a4a74-4264-bf55-646c-4e20be2d7373/mzaf_8180395547798189902.plus.aac.p.m4a'));
		            break;
                case 'air':
                    $bot->send(new VideoMessage($message['sender']['id'], 'http://dev1.social-gear.jp/sg_frontend_v2/videos/air.mp4'));
                    break;
                case 'louis test1':
                    $bot->send(new VideoMessage($message['sender']['id'], 'http://dev1.social-gear.jp/sg_frontend_v2/videos/louis2.3gp'));
                    break;

                 case 'l1':
                    $bot->send(new VideoMessage($message['sender']['id'], 'http://dev1.social-gear.jp/sg_frontend_v2/videos/l1.3gp'));
                    break;


                case 'l2':
                    $bot->send(new VideoMessage($message['sender']['id'], 'http://dev1.social-gear.jp/sg_frontend_v2/videos/l2.mp4'));
                    break;

             case 'l3':
                    $bot->send(new VideoMessage($message['sender']['id'], 'http://dev1.social-gear.jp/sg_frontend_v2/videos/l3.mp4'));
                    break;
                case 'missing':
                    $bot->send(new VideoMessage($message['sender']['id'], 'http://dev1.social-gear.jp/sg_frontend_v2/videos/air.mp4'));
                    break;
	            case 'guns':
		            $bot->send(new VideoMessage($message['sender']['id'], 'http://dev1.social-gear.jp/sg_frontend_v2/videos/guns.mp4'));
		            break;

                case '4':
                    $bot->send(new VideoMessage($message['sender']['id'], 'http://dev1.social-gear.jp/sg_frontend_v2/videos/4.mp4'));
                    break;
                case 'v3':
                    $bot->send(new VideoMessage($message['sender']['id'], 'http://c3544b7e.ngrok.io/testvideo/3.3gp'));
                    break;


	            case 'pdf':
		            $bot->send(new VideoMessage($message['sender']['id'], 'http://techslides.com/demos/sample-videos/small.mp4'));
		            break;
                    ...
                    $bot->setPersistentMenu([
                        new LocalizedMenu('default', false, [
                            new MenuItem(MenuItem::TYPE_NESTED, 'My Account', [
                               ...
                               // new MenuItem(MenuItem::TYPE_WEB, 'Contact Info', 'CONTACT_INFO_PAYLOAD')
                            ])
                        ])
                    ]);
                    break;
